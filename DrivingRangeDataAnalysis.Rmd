...
The pursuit of consistency is a central challenge for golfers at every skill level. A key aspect of this is understanding not just the average distance one hits each club, but also the factors that cause variation in those shots. Why does one 7-iron shot fly perfectly to the target while another lands short and 15 yards to the right?

This analysis delves into personal launch monitor data to identify the key swing metrics that influence a golf ball's flight and final resting position. By examining impact data such as Club Speed, Attack Angle, Club Path, and Club Face angle, the goal is to build a model that explains the deviation in shot outcomes to a significant degree.

The primary objective is to quantify the effect of these input signals on both the carry distance and the lateral deviation from the target line (by both degrees and total distance). By understanding which factors are most influential, a player can better focus their practice to achieve more consistent and predictable results on the golf course.
...
Now let's establish definitions for the non-obvious dimensions and measurements we'll be using:
...
_Example: The driver in this data set has 9° of loft, so it's Index.Number will be 1.09. Likewise, the lob wedge with 60° of loft will have an Index.Number of 5.60, while the 7 iron will have an Index.Number of 4.7._
...
We'll utilize several functions during our analysis to help us graph and evaluate our models. To limit the size of this document, we'll define these functions in the code chunk below:
...
```{r club-speed-boxplot, warning=FALSE, include=FALSE, results = 'hide'}
plot_boxplot(
  data = data,
  x_var = Club.Name,
  y_var = Club.Speed,
...
```
![Figure _. Box-and-whisker plots comparing the distributions of measured clubhead speed across golf clubs sampled.](README_files/figure-gfm/club-speed-boxplot-1.png)

```{r attack-angle-boxplot, warning=FALSE, include=FALSE, results = 'hide'}
plot_boxplot(
  data = data,
  x_var = Club.Name,
  y_var = Attack.Angle,
...
```
![Figure _. Box-and-whisker plots comparing the distributions of measured attack angle across golf clubs sampled.](README_files/figure-gfm/attack-angle-boxplot-1.png)

```{r club-face-and-path-histogram, warning=FALSE, include=FALSE, results = 'hide'}
...
p1 / p2
```
![Figure _. Frequency distributions of (a) club face angle and (b) club path angle at impact. ](README_files/figure-gfm/club-face-histogram-1.png)

### Building an Interpretable model

For our first model, we aim to build a model that prioritizes interpretability while maintaining decent predictive power and variation explained by our signals. We will pursue this by keeping the model small **TODO(ADD IN HOW SMALL? E.G. < 4 params)** while maintaining acceptable regression diagnostics (LINE assumptions).


Let's select the signal columns we'll use for our first iteration of the model. Some of these may be removed later due to collinearity issues. We are specifically choosing measurements that are "pre-ball flight" at first, artificially handicapping ourselves in order to first test whether we can achieve well-enough performing results without post-launch data.
```{r select-signals}
shot_signals = data[c(
...
Method: multiple linear regression using Least Squares 
...
```{r cor-matrix, message = F, warning = F}
...
```{r 2x2-grid, echo=FALSE, out.width="50%", fig.align="center", fig.show="hold"}
# Make sure you have the knitr library loaded
# You don't need to call library(knitr) here if it's already loaded

# Provide the file paths to your four images
knitr::include_graphics(c(
  "README_files/figure-gfm/3D-SimpleLinearModel-1.png",
  "README_files/figure-gfm/3D-SimpleLinearModel-2.png",
  "README_files/figure-gfm/3D-SimpleLinearModel-3.png",
  "README_files/figure-gfm/3D-SimpleLinearModel-4.png"
))
```
