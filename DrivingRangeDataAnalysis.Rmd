...
```{r club-speed-boxplot, warning=FALSE, include=FALSE}
plot_boxplot(
  data = data,
  x_var = Club.Name,
  y_var = Club.Speed,
...
```{r attack-angle-boxplot, warning=FALSE, include=FALSE}
plot_boxplot(
  data = data,
  x_var = Club.Name,
  y_var = Attack.Angle,
...
```{r club-face-histogram, warning=FALSE, include=FALSE}
...
```{r club-path-histogram, warning=FALSE, include=FALSE}
ggplot(data, aes(x = Club.Path)) +
  geom_histogram(binwidth = 1, fill = "dodgerblue", color = "black", alpha = 0.7) +
  geom_vline(xintercept = 0, color = "darkorange", linetype = "dashed", linewidth = 1) +
  labs(
    title = "Distribution of Club Path",
